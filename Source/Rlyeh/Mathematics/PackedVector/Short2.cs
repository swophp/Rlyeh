...
using static Rlyeh.Mathematics.VectorUtilities;
...
public readonly struct Short2 : IPackedVector<uint>, IEquatable<Short2>
{
    ...
    public Short2(float x, float y)
    {
        ...
        var vector = Vector128.Create(x, y, 0.0f, 0.0f);
        if (Sse41.IsSupported)
        {
            // Bounds check
            var result = Clamp(vector, ShortMin, ShortMax);
            // Convert to int with rounding
            var vInt = Sse2.ConvertToVector128Int32(result);
            // Pack the ints into shorts
            var vShort = Sse2.PackSignedSaturate(vInt, vInt);
        ...
        {
            var result = AdvSimd.Max(vector, ShortMin);
            result = AdvSimd.Min(result, AdvSimd.DuplicateToVector128(32767.0f));
            var vInt32 = AdvSimd.ConvertToInt32RoundToZero(result);
            var vInt16 = AdvSimd.ExtractNarrowingSaturateLower(vInt32);
