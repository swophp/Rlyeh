...
public struct BoundingSphere : IEquatable<BoundingSphere>, IFormattable
{
    ...
    public static BoundingSphere CreateFromPoints(Span<Vector3> points)
    {
        ...
        var DeltaX = Vector3.Subtract(MaxX, MinX);
        var DeltaY = Vector3.Subtract(MaxY, MinY);
        var DeltaZ = Vector3.Subtract(MaxZ, MinZ);

        var DistX = DeltaX.Length();
        var DistY = DeltaY.Length();
        var DistZ = DeltaZ.Length();
        ...
        for (var i = 0; i < points.Length; ++i)
        {
            ...
            var Delta = Vector3.Subtract(point, center);
    ...
    {
        var center1 = original._center;
        var r1 = original._radius;

        var center2 = additional._center;
        var r2 = additional._radius;

        var V = Vector3.Subtract(center2, center1);
        ...
        var N = Vector3.Divide(V, d);

        var t1 = MathF.Min(-r1, d - r2);
        var t2 = MathF.Max(r1, d + r2);
        var t_5 = (t2 - t1) * 0.5f;

        var NCenter = Vector3.Add(center1, N * (t_5 + t1));
    ...
    public static void Transform(in BoundingSphere sphere, in Matrix4x4 transform, out BoundingSphere result)
    {
        ...
        var radius = sphere.Radius * (float)Math.Sqrt(majorAxisLengthSquared);
    ...
    {
        var distance = Vector3.Distance(Center, sphere.Center);
    ...
    public readonly float? Intersects(in Ray ray)
    {
        ...
        var m = Vector3.Subtract(ray.Position, Center);
        ...
        var distance = -b - (float)Math.Sqrt(discriminant);
    ...
    {
        var clampedVector = Vector3.Clamp(Center, box.Min, box.Max);
    ...
    {
        var radiisum = Radius + sphere.Radius;
    ...
    /// <inheritdoc/>
    public readonly override bool Equals(object? obj) => obj is BoundingSphere other && Equals(other);
