...
// The original code is Copyright (c) Microsoft. All rights reserved. Licensed under the MIT License (MIT).

using static Rlyeh.Mathematics.MathHelper;
...
public static unsafe class VectorUtilities
{
    ...
    public static Vector128<float> Round(Vector128<float> vector)
    {
        ...
        {
            var sign = Sse.And(vector, NegativeZero.AsSingle());
            var sMagic = Sse.Or(NoFraction, sign);
            var R1 = Sse.Add(vector, sMagic);
            R1 = Sse.Subtract(R1, sMagic);
            var R2 = Sse.And(vector, AbsMask.AsSingle());
            var mask = Sse.CompareLessThanOrEqual(R2, NoFraction);
            R2 = Sse.AndNot(mask, vector);
            R1 = Sse.And(R1, mask);
            var result = Sse.Xor(R1, R2);
            return result;
        }
        else if (AdvSimd.IsSupported)
        {
    ...
    public static Vector128<float> Clamp(Vector128<float> vector, Vector128<float> min, Vector128<float> max)
    {
        ...
        if (Sse.IsSupported)
        {
            var result = Sse.Max(min, vector);
            result = Sse.Min(max, result);
            return result;
        }
        else if (AdvSimd.IsSupported)
        {
            var result = AdvSimd.Max(min, vector);
            result = AdvSimd.Min(max, result);
            return result;
        }
        else
        {
            var result = Max(min, vector);
    ...
    public static Vector128<float> Saturate(Vector128<float> vector)
    {
        if (Sse.IsSupported)
        {
            var result = Sse.Max(vector, Vector128<float>.Zero);
            result = Sse.Min(result, One);
            return result;
        }
        else if (AdvSimd.IsSupported)
        {
            var result = AdvSimd.Max(vector, Vector128<float>.Zero);
